...
describe('app view account usage badge', () => {
  it('uses localized Chinese time labels while preserving metric terms', () => {
    const copy = messages['zh-CN']

    expect(`${copy.today} ${copy.tokens}`).toBe('今日 tokens')
    expect(`${copy.last90Days} ${copy.cost}`).toBe('近 90 天 用量')
    expect(copy.instanceUsageDescription).toContain('近 30 天 tokens')
    expect(copy.instanceUsageDescription).not.toMatch(/\b(Today|Last 30 days)\b/)
  })
