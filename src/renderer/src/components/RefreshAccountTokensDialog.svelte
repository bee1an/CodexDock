...
<style>
  ...
  /* Dark mode */
  :global(html[data-theme='dark']) .refresh-status-idle {
    background: rgba(255, 255, 255, 0.08);
    color: var(--ink-soft-strong);
  }

  :global(html[data-theme='dark']) .refresh-status-running {
    background: rgb(59 130 246 / 0.16);
    color: rgb(147 197 253);
  }

  :global(html[data-theme='dark']) .refresh-status-success {
    background: rgb(16 185 129 / 0.16);
    color: rgb(167 243 208);
  }

  :global(html[data-theme='dark']) .refresh-status-error {
    background: rgb(239 68 68 / 0.16);
    color: rgb(252 165 165);
  }

  :global(html[data-theme='dark']) .refresh-card {
    border-color: color-mix(in srgb, var(--color-arctic-mist) 78%, transparent);
    background: color-mix(in srgb, var(--panel-strong) 78%, var(--color-snow));
  }

  :global(html[data-theme='dark']) .refresh-card-row dd {
    color: rgba(255, 255, 255, 0.88);
  }

  :global(html[data-theme='dark']) .refresh-error {
    border-color: rgb(239 68 68 / 0.24);
    background: rgb(239 68 68 / 0.1);
  }

  :global(html[data-theme='dark']) .refresh-error-icon {
    color: rgb(252 165 165);
  }

  :global(html[data-theme='dark']) .refresh-error-text {
    color: rgb(252 165 165);
  }

  :global(html[data-theme='dark']) .refresh-log-panel {
    border-color: color-mix(in srgb, var(--color-arctic-mist) 78%, transparent);
    background: color-mix(in srgb, var(--panel-strong) 78%, var(--color-snow));
  }

  :global(html[data-theme='dark']) .refresh-raw-toggle:hover {
    color: var(--color-carbon);
