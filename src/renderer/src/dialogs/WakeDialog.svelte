<script lang="ts">
  ...
  import { formatWakeScheduleLastTriggeredAt, nextWakeScheduleLabel } from '$lib/view/wake-schedule'
  ...
  export let scheduleDisabled = false
  export let scheduleDisabledReason = ''
  ...
  export let onSubmitAuto: () => void | Promise<void> = () => {}
  ...
  <div class="grid gap-4">
    ...
    >
      ...
      <AppButton
        ...
        onclick={() => {
          if (!dialogBusy() && !scheduleDisabled) {
            activeTab = 'schedule'
          }
        }}
        disabled={dialogBusy() || scheduleDisabled}
        ariaPressed={activeTab === 'schedule'}
        title={scheduleDisabled ? scheduleDisabledReason : undefined}
      >
    ...
    {#if activeTab === 'session'}
      <div class="grid gap-4" data-wake-motion use:reveal={{ delay: 0.02 }}>
        ...
        <div class="flex justify-end gap-2" data-wake-motion>
          ...
          <AppButton variant="secondary" size="sm" onclick={onSubmitAuto} disabled={dialogBusy()}>
            {#if sessionBusy}
              <span class="i-lucide-loader-circle h-4 w-4 animate-spin"></span>
            {/if}
            <span>{copy.wakeQuotaAuto}</span>
          </AppButton>
      ...
      <div class="grid gap-4" data-wake-motion use:reveal={{ delay: 0.02 }}>
        ...
        >
          <div class="grid gap-1">
            ...
            {#if scheduleDisabled}
              <p class="text-xs leading-5 text-danger">{scheduleDisabledReason}</p>
            {/if}
          </div>
          <label class="inline-flex items-center gap-2 text-xs font-medium text-carbon">
            <Checkbox bind:checked={scheduleEnabled} disabled={dialogBusy() || scheduleDisabled} />
