...
describe('AccountsListView', () => {
  ...
  it('searches accounts in the current account list', async () => {
    renderAccountsListView()

    await fireEvent.input(screen.getByRole('searchbox', { name: copy.accountSearchAria }), {
  ...
  it('sorts visible accounts by usage and persists the order', async () => {
    ...
    await fireEvent.click(screen.getByRole('button', { name: copy.accountSortByUsage }))
    await fireEvent.click(screen.getByRole('button', { name: copy.accountSortPrimaryDesc }))

    await waitFor(() => {
      expect(reorderAccounts).toHaveBeenCalledWith(['acct-2', 'acct-1'])
    })
  })
