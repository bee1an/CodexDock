<script lang="ts">
  ...
  import {
    ...
    sortAccountsByUsage,
    type AccountUsageSortDirection,
    type AccountUsageSortField,
    ungroupedFilterId,
    visibleAccountsForFilter
  } from './accounts-panel-account'
  import { animateProgress } from '$lib/motion/gsap-motion'
  import {
  ...
  import AppPopover from '$lib/ui/AppPopover.svelte'
  ...
  let usageSortMenuOpen = false
  let usageSortMenuAnchorRect: DOMRect | null = null
  let usageSortTriggerNode: HTMLElement | null = null
  let usageSortSaving = false
  ...
  const usageSortOptions: Array<{
  ...
  }> = [
    {
      ...
      label: () => copy.accountSortPrimaryAsc
    },
    {
      field: 'primary',
      direction: 'desc',
    ...
    },
    {
      ...
      direction: 'asc',
      icon: 'i-lucide-arrow-up-narrow-wide',
      label: () => copy.accountSortSecondaryAsc
    },
    {
      field: 'secondary',
      direction: 'desc',
  ...
  function hasEnabledWakeSchedule(accountId: string): boolean {
    ...
    return Boolean(
      schedule?.enabled && schedule.times.length && !isFreePlan(usageByAccountId[accountId])
    )
  }
  ...
  function accountHealthTitle(health: AccountHealth | undefined): string {
    ...
    const lines = [
      health.status === 'rate_limited'
        ? copy.accountHealthRateLimitedHint(health.reason)
    ...
    if (health.retryAt) {
      lines.push(`${copy.accountHealthRetryAt}: ${health.retryAt}`)
    }
  ...
  function accountHealthLabel(health: AccountHealth | undefined): string {
    if (!health) {
      return copy.accountHealthNormal
    }
    return health.status === 'rate_limited'
      ? copy.accountHealthRateLimited
      : copy.accountHealthAuthError
  }
  ...
  async function applyUsageSort(
    field: AccountUsageSortField,
    direction: AccountUsageSortDirection
  ): Promise<void> {
    if (loginActionBusy || groupMutationBusy || usageSortSaving || !visibleAccounts.length) {
      return
    }

    const sortedAccounts = sortAccountsByUsage(visibleAccounts, usageByAccountId, field, direction)
    const order = buildPersistedUsageSortOrder(
      activeGroupFilter,
      groupFilteredAccounts,
      sortedAccounts,
      groups
    ...
    usageSortSaving = true
    try {
      if (order.type === 'group' && order.groupId) {
        await reorderAccountsInGroup(order.groupId, order.accountIds)
      } else {
        await reorderAccounts(order.accountIds)
      }
    } finally {
      usageSortSaving = false
      usageSortMenuOpen = false
      usageSortMenuAnchorRect = null
    }
  }
  ...
  function closeUsageSortMenu(): void {
    usageSortMenuOpen = false
    usageSortMenuAnchorRect = null
  }
...
>
  ...
  <div class="ml-auto flex items-center gap-1.5">
    ...
    <AppButton
      variant="secondary"
      size="xs"
      onclick={openWakeAllDialog}
      disabled={loginActionBusy || wakeAllBusy || !visibleAccounts.length}
      ariaLabel={copy.wakeAllAccounts}
      title={copy.wakeAllAccounts}
    >
      <span
    ...
    <div class="grid gap-2">
      <div class="flex flex-wrap items-center justify-between gap-2">
        <div class="relative min-w-[14rem] flex-1">
          <AppInput
            ...
            ariaLabel={copy.accountSearchAria}
            disabled={loginActionBusy || usageSortSaving}
          />
          {#if accountSearchDraft}
            <button
              ...
              aria-label={copy.accountSearchClear}
              disabled={loginActionBusy || usageSortSaving}
              onclick={() => {
                accountSearchDraft = ''
              }}
        ...
        <div class="flex flex-wrap items-center gap-1.5">
          <span class="theme-version-pill rounded-md px-2 py-0.5 text-[10px] text-muted-strong">
            {copy.accountSearchResult(visibleAccounts.length, groupFilteredAccounts.length)}
          </span>
          <div class="relative">
            <AppButton
              variant="secondary"
              ...
              class="gap-1.5"
              onclick={(e) => {
                if (usageSortMenuOpen) {
                  usageSortMenuOpen = false
                  usageSortMenuAnchorRect = null
                  return
                }
                usageSortTriggerNode = e.currentTarget as HTMLElement
                usageSortMenuAnchorRect = usageSortTriggerNode.getBoundingClientRect()
                usageSortMenuOpen = true
              }}
              disabled={loginActionBusy ||
                groupMutationBusy ||
                usageSortSaving ||
                !visibleAccounts.length}
              ariaLabel={copy.accountSortByUsage}
              title={copy.accountSortByUsage}
            >
              <span
                class={`${usageSortSaving ? 'i-lucide-loader-circle animate-spin' : 'i-lucide-arrow-up-down'} h-3.5 w-3.5`}
              ></span>
              <span>{copy.accountSortByUsage}</span>
            </AppButton>
            <AppPopover
              open={usageSortMenuOpen}
              anchorRect={usageSortMenuAnchorRect}
              align="end"
              ignoreNode={usageSortTriggerNode}
              class="min-w-[13rem] rounded-[0.55rem] p-1"
              onclose={() => {
                usageSortMenuOpen = false
                usageSortMenuAnchorRect = null
              }}
            >
              <p class="px-3 py-1 text-[10px] font-medium uppercase tracking-wide text-faint">
                {copy.accountSortByUsage}
              </p>
              {#each usageSortOptions as option (`${option.field}:${option.direction}`)}
                <button
                  class="flex w-full items-center gap-2 rounded-[0.35rem] border-0 bg-transparent px-3 py-1.5 text-left text-xs text-carbon transition-colors hover:bg-[var(--surface-soft)] disabled:cursor-not-allowed disabled:opacity-50"
                  type="button"
                  disabled={usageSortSaving}
                  onclick={() => void applyUsageSort(option.field, option.direction)}
                >
                  <span class={`${option.icon} h-3.5 w-3.5 text-muted-strong`}></span>
      ...
      <div class="flex items-center justify-between gap-2">
        ...
        <AppButton
          ...
          disabled={loginActionBusy || groupMutationBusy || usageSortSaving}
      ...
      <div class="flex flex-wrap items-center justify-end gap-1.5">
        {#if visibleAccounts.length && !allVisibleSelected}
          <AppButton
            ...
            disabled={loginActionBusy || usageSortSaving || !visibleAccounts.length}
          >
        ...
        <AppButton
          ...
          disabled={loginActionBusy || usageSortSaving || removingSelection}
        >
  ...
  <div class="min-h-0 flex-1 overflow-y-auto px-4">
    <div
      ...
      use:dragHandleZone={{
        ...
        flipDurationMs,
        dragDisabled:
          loginActionBusy || groupMutationBusy || dragSortDisabled || sortableAccounts.length < 2,
            ...
            <div class="mt-[-2px] flex min-w-0 flex-wrap items-center gap-1.5">
              ...
              {#if healthBlocked}
                ...
                >
                  ...
                  <span class="truncate">{accountHealthLabel(health)}</span>
                </span>
              {/if}
