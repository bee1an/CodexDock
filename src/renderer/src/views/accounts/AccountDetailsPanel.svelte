<script lang="ts">
  ...
  import { planLabel } from '$lib/view/app-view'
  ...
  $: healthRetryAt = health?.retryAt
    ? formatTimestampWithRelative(health.retryAt, language, timestampCopy, tick)
    : ''
  ...
  function healthTitle(): string {
    ...
    if (healthRetryAt) {
      lines.push(`${copy.accountHealthRetryAt}: ${healthRetryAt}`)
    }
          ...
          <div class="flex min-w-0 flex-col">
            ...
            {#if health}
              <span class="truncate text-[11px] text-faint" title={health.reason}>
                {health.reason} · {healthRetryAt || healthMarkedAt}
              </span>
            {/if}
          </div>
        </dd>
