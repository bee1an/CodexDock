<script lang="ts">
  ...
  import {
    ...
    type AccountConsumptionEntry,
    type GroupConsumptionEntry
  } from '../stats/cost-stats-data'

  Chart.register(BarController, BarElement, CategoryScale, LinearScale, Tooltip, Legend)

  type StatusFilter = 'all' | 'ok' | 'warn' | 'error'
  type AccountUsageSortKey = 'last30DaysTokens' | 'todayTokens' | 'last30DaysCostUSD' | 'updatedAt'
  type AccountUsageMode = 'account' | 'group'
  type UsageConsumptionEntry = AccountConsumptionEntry | GroupConsumptionEntry
  ...
  export let updateAutoStart: (autoStart: boolean) => Promise<void> = async () => {}
  export let visibleColumns: string[] | undefined = undefined
  export let updateVisibleColumns: (columns: string[]) => Promise<void> = async () => {}
  export let gatewayUsageByAccountId: Record<string, GatewayUsageSummary> = {}
  ...
  let visibleColumnSet: Set<LogColumn> = new SvelteSet(
    visibleColumns?.length ? allColumns.filter((col) => visibleColumns!.includes(col)) : allColumns
  )
  ...
  onDestroy(() => {
    ...
    accountUsageChart?.destroy()
    accountUsageChart = null
  })

  let accountUsageAllRows: AccountConsumptionEntry[] = []
  let accountUsageRows: AccountConsumptionEntry[] = []
  let groupUsageRows: GroupConsumptionEntry[] = []
  let accountUsageDisplayRows: UsageConsumptionEntry[] = []
  let accountUsageSortedRows: UsageConsumptionEntry[] = []
  let accountUsageCanvas: HTMLCanvasElement | null = null
  let accountUsageChart: Chart<'bar', number[], string> | null = null
  let accountUsageChartHeight = 280
  let accountUsageChartSyncKey = ''
  let accountUsageMode: AccountUsageMode = 'account'
  let accountUsageGroupDrilldownId = ''
  let accountUsageSortKey: AccountUsageSortKey = 'last30DaysTokens'
  let accountUsageSortOptions: Array<{ key: AccountUsageSortKey; label: string }> = []
  let accountUsageHoverIndex = -1
  let accountUsageHoverEntry: UsageConsumptionEntry | null = null
  let accountUsageTooltipX = 0
  let accountUsageTooltipY = 0

  const formatTokens = (value: number): string => new Intl.NumberFormat(undefined).format(value)
  ...
  const timestampScore = (value?: string): number => {
    if (!value) return -1
  ...
  const withAlpha = (hex: string, alpha: number): string => {
    ...
    if (!normalized.startsWith('#') || normalized.length < 7) return normalized
  ...
  const accountUsageSortScore = (
    entry: UsageConsumptionEntry,
    sortKey: AccountUsageSortKey
  ): number => {
    if (sortKey === 'last30DaysTokens') return entry.last30DaysTokens
    if (sortKey === 'todayTokens') return entry.sessionTokens
    if (sortKey === 'last30DaysCostUSD') return entry.last30DaysCostUSD ?? 0
    return timestampScore(entry.updatedAt)
  }

  const accountUsageChartValue = (entry: UsageConsumptionEntry): number => {
    if (accountUsageSortKey === 'todayTokens') return entry.sessionTokens
    if (accountUsageSortKey === 'last30DaysCostUSD') return entry.last30DaysCostUSD ?? 0
    return entry.last30DaysTokens
  }

  const formatAccountUsageChartValue = (value: number): string =>
    accountUsageSortKey === 'last30DaysCostUSD' ? formatCost(value) : formatTokens(value)

  const accountUsageMetricLabel = (): string => {
    if (accountUsageSortKey === 'todayTokens') return `${copy.today} ${copy.tokens}`
    if (accountUsageSortKey === 'last30DaysCostUSD') return `${copy.last90Days} ${copy.cost}`
    return `${copy.last90Days} ${copy.tokens}`
  }

  const accountUsageGroupName = (groupId: string): string =>
    groups.find((group) => group.id === groupId)?.name ?? groupId

  const accountBelongsToGroup = (accountId: string, groupId: string): boolean =>
    Boolean(accounts.find((account) => account.id === accountId)?.groupIds.includes(groupId))

  const drillIntoUsageGroup = (groupId: string): void => {
    clearAccountUsageHover()
    accountUsageGroupDrilldownId = groupId
    accountUsageMode = 'account'
  }

  const leaveUsageGroupDrilldown = (): void => {
    clearAccountUsageHover()
    accountUsageGroupDrilldownId = ''
    accountUsageMode = 'group'
  }

  const clearAccountUsageHover = (): void => {
    accountUsageHoverIndex = -1
  }

  const accountUsageIndexAtEvent = (event: MouseEvent | PointerEvent): number => {
    if (!accountUsageCanvas || !accountUsageChart) return -1
    const element = accountUsageChart.getElementsAtEventForMode(
      event,
      'nearest',
      { intersect: true },
      true
    ...
    return Number.isInteger(index) && index >= 0 && index < accountUsageSortedRows.length
      ? index
      : -1
  }

  const handleAccountUsageCanvasClick = (event: MouseEvent): void => {
    if (accountUsageMode !== 'group' || accountUsageGroupDrilldownId) return
    const item = accountUsageSortedRows[accountUsageIndexAtEvent(event)]
    if (item && 'groupId' in item) {
      drillIntoUsageGroup(item.groupId)
    }
  }

  const handleAccountUsageCanvasPointerMove = (event: PointerEvent): void => {
    const index = accountUsageIndexAtEvent(event)
    accountUsageHoverIndex = index
    if (index < 0 || !accountUsageCanvas) return

    const rect = accountUsageCanvas.getBoundingClientRect()
    const maxTooltipX = Math.max(8, rect.width - 220)
    accountUsageTooltipX = Math.min(Math.max(event.clientX - rect.left + 12, 8), maxTooltipX)
    accountUsageTooltipY = Math.max(event.clientY - rect.top, 16)
  }

  const accountUsageTooltipMetricValue = (entry: UsageConsumptionEntry): string =>
    accountUsageSortKey === 'last30DaysCostUSD'
      ? formatCost(entry.last30DaysCostUSD)
      : formatAccountUsageChartValue(accountUsageChartValue(entry))

  const accountUsageTooltipLines = (entry: UsageConsumptionEntry): string[] => {
    const lines = [
      `${copy.last90Days} ${copy.tokens}: ${formatTokens(entry.last30DaysTokens)}`,
      `${copy.today} ${copy.tokens}: ${formatTokens(entry.sessionTokens)}`,
      `${copy.last90Days} ${copy.cost}: ${formatCost(entry.last30DaysCostUSD)}`,
      `${copy.today} ${copy.cost}: ${formatCost(entry.sessionCostUSD)}`,
      `${copy.updatedAt}: ${formatUpdatedAt(entry.updatedAt)}`
    ]
    if ('accountCount' in entry) {
      lines.push(copy.accountCount(entry.accountCount))
    }
    return lines
  }

  const sortAccountUsageRows = (
    rows: UsageConsumptionEntry[],
    sortKey: AccountUsageSortKey
  ): UsageConsumptionEntry[] =>
    [...rows].sort((left, right) => {
      const scoreDelta =
        accountUsageSortScore(right, sortKey) - accountUsageSortScore(left, sortKey)
      if (scoreDelta !== 0) return scoreDelta
      if (right.last30DaysTokens !== left.last30DaysTokens) {
  ...
  const syncAccountUsageChart = (): void => {
    if (!accountUsageCanvas) return
    if (!accountUsageSortedRows.length) {
      accountUsageChart?.destroy()
      accountUsageChart = null
      return
    }

    const context = accountUsageCanvas.getContext('2d')
    if (!context) return
    ...
    const metricLabel = accountUsageMetricLabel()
    const barColor = withAlpha(accent, 0.72)

    const config: ChartConfiguration<'bar', number[], string> = {
      type: 'bar',
      data: {
        labels: accountUsageSortedRows.map((entry) => entry.label),
        datasets: [
          {
            label: metricLabel,
            data: accountUsageSortedRows.map((entry) => accountUsageChartValue(entry)),
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            ...
            callbacks: {
              ...
              label: (ctx) => {
                const item = accountUsageSortedRows[ctx.dataIndex]
                if (!item) return ''
                const metricValue =
                  accountUsageSortKey === 'last30DaysCostUSD'
                    ? formatCost(item.last30DaysCostUSD)
                    : formatAccountUsageChartValue(accountUsageChartValue(item))
              ...
              afterLabel: (ctx) => {
                ...
                if (!item) return ''
                const lines = [
                  `${copy.last90Days} ${copy.tokens}: ${formatTokens(item.last30DaysTokens)}`,
                  `${copy.today} ${copy.tokens}: ${formatTokens(item.sessionTokens)}`,
                  `${copy.last90Days} ${copy.cost}: ${formatCost(item.last30DaysCostUSD)}`,
                  `${copy.today} ${copy.cost}: ${formatCost(item.sessionCostUSD)}`,
                  `${copy.updatedAt}: ${formatUpdatedAt(item.updatedAt)}`
                ]
                if ('accountCount' in item) {
                  lines.push(copy.accountCount(item.accountCount))
                }
                return lines
              }
            }
        ...
        scales: {
          x: {
            beginAtZero: true,
            grid: { color: withAlpha(line, 0.55) },
            ticks: {
              color: muted,
              maxTicksLimit: 5,
              callback: (value) => formatAccountUsageChartValue(Number(value))
            }
          },
          y: {
            grid: { display: false },
            ticks: {
              ...
              callback: (value) =>
                truncateChartLabel(accountUsageSortedRows[Number(value)]?.label ?? String(value))
            }
          }
        }
      }
    ...
    if (accountUsageChart) {
      accountUsageChart.data = config.data
      accountUsageChart.options = config.options ?? {}
      accountUsageChart.update()
      return
    }
    accountUsageChart = new Chart(context, config)
  }

  $: accountUsageAllRows = buildAccountConsumptionEntriesFromGateway({
    gatewayUsageByAccountId,
    accounts,
    resolveLabel: (accountId, account) => (account ? formatAccountLabel(account) : accountId)
  })
  $: accountUsageRows = accountUsageGroupDrilldownId
    ? accountUsageAllRows.filter((entry) =>
        accountBelongsToGroup(entry.accountId, accountUsageGroupDrilldownId)
      )
    : accountUsageAllRows
  $: groupUsageRows = buildGroupConsumptionEntriesFromGateway({
    gatewayUsageByAccountId,
    accounts,
    groups,
    resolveLabel: (groupId, group) => group?.name ?? groupId
  })
  $: if (
    accountUsageGroupDrilldownId &&
    !groups.some((group) => group.id === accountUsageGroupDrilldownId)
  ) {
    accountUsageGroupDrilldownId = ''
  }
  $: if (accountUsageMode === 'group' && !groupUsageRows.length && accountUsageRows.length) {
    accountUsageMode = 'account'
  }
  $: accountUsageDisplayRows =
    accountUsageMode === 'group' && !accountUsageGroupDrilldownId
      ? groupUsageRows
      : accountUsageRows
  $: accountUsageSortOptions = [
    { key: 'last30DaysTokens', label: `${copy.last90Days} ${copy.tokens}` },
    { key: 'todayTokens', label: `${copy.today} ${copy.tokens}` },
    { key: 'last30DaysCostUSD', label: `${copy.last90Days} ${copy.cost}` },
    { key: 'updatedAt', label: copy.updatedAt }
  ]
  $: accountUsageSortedRows = sortAccountUsageRows(accountUsageDisplayRows, accountUsageSortKey)
  $: accountUsageHoverEntry =
    accountUsageHoverIndex >= 0 ? (accountUsageSortedRows[accountUsageHoverIndex] ?? null) : null
  $: if (accountUsageHoverIndex >= accountUsageSortedRows.length) {
    clearAccountUsageHover()
  }
  $: accountUsageChartHeight = Math.max(280, accountUsageSortedRows.length * 34)
  $: accountUsageChartSyncKey = [
    accountUsageSortKey,
    accountUsageMode,
    accountUsageGroupDrilldownId,
    ...
    copy.cost,
    copy.updatedAt,
    ...accountUsageSortedRows.map(
      (entry) =>
        `${entry.label}:${entry.sessionTokens}:${entry.sessionCostUSD ?? ''}:${entry.last30DaysTokens}:${entry.last30DaysCostUSD ?? ''}:${entry.updatedAt}`
    )
  ].join('|')
  $: if (!accountUsageCanvas && accountUsageChart) {
    accountUsageChart.destroy()
    accountUsageChart = null
  }
  $: if (accountUsageCanvas && accountUsageChartSyncKey) {
    syncAccountUsageChart()
  }
</script>
...
<div class="gateway-container flex min-h-0 flex-1 flex-col gap-4 px-4 pb-4">
  ...
  >
    ...
    <div class="flex flex-wrap items-center justify-end gap-1.5">
      <label
        class="gateway-auto-start-toggle inline-flex items-center gap-1.5 rounded-[0.35rem] border border-[var(--card-border)] bg-[var(--surface-soft)] px-2 py-1.5 text-[11px] text-muted-strong"
        title={copy.localGatewayAutoStartHint}
      >
        <Checkbox
          checked={autoStart}
          disabled={autoStartBusy}
          ariaLabel={copy.localGatewayAutoStart}
          onCheckedChange={(checked) => void toggleAutoStart(checked)}
        />
        <span>{copy.localGatewayAutoStart}</span>
      </label>
  ...
  <div class="gateway-scroll flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto overflow-x-hidden">
    ...
    <section
      class="gateway-panel gateway-usage-panel rounded-[0.45rem] border"
      aria-labelledby="local-gateway-usage-heading"
    >
      <div
        class="gateway-mappings-header flex flex-wrap items-center justify-between gap-3 border-b px-3 py-2.5"
      >
        <div class="flex min-w-0 flex-col gap-1.5">
          {#if accountUsageGroupDrilldownId}
            <AppButton
              variant="ghost"
              size="xs"
              class="w-fit pl-0"
              onclick={leaveUsageGroupDrilldown}
              ariaLabel={copy.accountUsageBackToGroups}
            >
              <span class="i-lucide-arrow-left h-3.5 w-3.5" aria-hidden="true"></span>
              <span>{copy.accountUsageBackToGroups}</span>
            </AppButton>
          {/if}
          <div class="flex min-w-0 items-center gap-2">
            <span
              ...
              aria-hidden="true"
            >
              <span class="i-lucide-bar-chart-3 h-3.5 w-3.5"></span>
            </span>
            <div class="min-w-0">
              <h3 id="local-gateway-usage-heading" class="text-[12px] font-semibold text-carbon">
                {copy.accountUsageOverview}
              </h3>
              <p class="mt-0.5 text-[10px] leading-4 text-faint">
                {accountUsageGroupDrilldownId
                  ? copy.accountUsageGroupDrilldown(
                      accountUsageGroupName(accountUsageGroupDrilldownId)
                    )
                  : copy.accountUsageOverviewDescription}
              </p>
            </div>
          </div>
        </div>

        {#if accountUsageRows.length || groupUsageRows.length}
          <div class="flex flex-wrap items-center justify-end gap-1.5">
            <span
              class="gateway-status-pill inline-flex flex-none items-center rounded border px-1.5 py-0.5 text-[9px] font-mono font-medium tabular-nums text-muted-strong"
            >
              {accountUsageMode === 'group' && !accountUsageGroupDrilldownId
                ? copy.groupCount(groupUsageRows.length)
                : copy.accountCount(accountUsageRows.length)}
            </span>
            <AppButton
              variant="filter"
              size="xs"
              selected={accountUsageMode === 'account'}
              ariaPressed={accountUsageMode === 'account'}
              onclick={() => {
                accountUsageGroupDrilldownId = ''
                accountUsageMode = 'account'
              }}
            >
              {copy.accountUsageByAccount}
            </AppButton>
            <AppButton
              variant="filter"
              size="xs"
              selected={accountUsageMode === 'group'}
              ariaPressed={accountUsageMode === 'group'}
              disabled={!groupUsageRows.length}
              onclick={() => {
                accountUsageGroupDrilldownId = ''
                accountUsageMode = 'group'
              }}
            >
              {copy.accountUsageByGroup}
            </AppButton>
            {#each accountUsageSortOptions as option (option.key)}
              <AppButton
                variant="filter"
                size="xs"
                selected={accountUsageSortKey === option.key}
                ariaPressed={accountUsageSortKey === option.key}
                onclick={() => {
                  accountUsageSortKey = option.key
                }}
              >
                {option.label}
              </AppButton>
      ...
      {#if accountUsageSortedRows.length}
        <div class="px-3 py-3">
          <div class="account-usage-chart-frame rounded-[0.35rem] border">
            <div class="account-usage-chart-scroll overflow-y-auto overflow-x-hidden px-3 py-3">
              <div
                class="account-usage-chart-canvas relative"
                style={`height: ${accountUsageChartHeight}px`}
              >
                <canvas
                  bind:this={accountUsageCanvas}
                  aria-label={copy.accountUsageOverview}
                  onclick={handleAccountUsageCanvasClick}
                  onpointermove={handleAccountUsageCanvasPointerMove}
                  onpointerleave={clearAccountUsageHover}
                  style={accountUsageMode === 'group' && !accountUsageGroupDrilldownId
                    ? 'cursor: pointer'
                    : undefined}
                ></canvas>
                {#if accountUsageHoverEntry}
                  <div
                    class="account-usage-tooltip pointer-events-none absolute z-10 rounded-[0.35rem] border px-3 py-2 text-[10px] shadow-lg"
                    style={`left: ${accountUsageTooltipX}px; top: ${accountUsageTooltipY}px; transform: translateY(-50%);`}
                    role="tooltip"
                  >
                    <div class="max-w-[14rem] truncate text-[11px] font-semibold text-carbon">
                      {accountUsageHoverEntry.label}
                    </div>
                    <div class="mt-1 font-mono text-[10px] font-semibold text-carbon tabular-nums">
                      {accountUsageMetricLabel()}: {accountUsageTooltipMetricValue(
                        accountUsageHoverEntry
                      )}
                    </div>
                    <div class="mt-1 grid gap-0.5 text-faint">
                      {#each accountUsageTooltipLines(accountUsageHoverEntry) as line, index (`${index}:${line}`)}
                        <div>{line}</div>
                      {/each}
                    </div>
                  </div>
      ...
      {:else}
        <div class="flex flex-col items-center gap-2 px-4 py-8 text-center">
          <span class="i-lucide-bar-chart-3 h-7 w-7 opacity-15"></span>
          <p class="text-[12px] font-semibold text-carbon">{copy.accountUsageNoData}</p>
          <p class="max-w-sm text-[11px] text-faint">{copy.accountUsageUnavailable}</p>
        </div>
      {/if}
    </section>
  </div>
...
<style>
  ...
  .account-usage-chart-frame {
    border-color: color-mix(in srgb, var(--line-strong) 72%, transparent);
    background: color-mix(in srgb, var(--panel-strong) 96%, transparent);
    box-shadow: inset 0 1px 0 color-mix(in srgb, var(--edge-light) 40%, transparent);
  }

  .account-usage-chart-scroll {
    max-height: 360px;
    scrollbar-gutter: stable;
  }

  .account-usage-chart-canvas {
    min-width: 0;
    width: 100%;
  }

  .account-usage-chart-canvas canvas {
    display: block;
    width: 100%;
    height: 100%;
  }
  ...
  .account-usage-tooltip {
    ...
    box-shadow:
      0 14px 36px color-mix(in srgb, var(--color-carbon) 14%, transparent),
      inset 0 1px 0 color-mix(in srgb, var(--edge-light) 42%, transparent);
  }
